                    ...
                    <div className=' flex w-full bg-slate-200  '>
                        {loading ? (
                            ...
                            <div className='bg-gray-100 m-auto w-[90%] justify-center'>
                                ...
                                {forumData && forumData.map((data) => (
                                    <form key={data._id} >
                                        <div className="w-[80%] m-auto mt-[2%] border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                                            <div className='flex border-gray-500 border-b-2'>

                                                <img src={data.ID.photo ?  data.ID.photo : "https://static.vecteezy.com/system/resources/thumbnails/024/226/462/small/happy-anime-boy-logo-vector.jpg"}
                                                 className='rounded-full w-[30px] h-[30px] object-cover m-2 ' alt="" />
                                                <p className='ml-[10px] m-auto font-semibold'>{data.ID.name}</p>
                                            </div>
                                            <div className="px-4 py-2 bg-white rounded-t-lg dark:bg-gray-800 max-h-[95px] overflow-hidden ">
                                                <p className='font-bold mb-2'>{data.comment}</p>
                                            </div>
                                            <div className='flex w-full justify-end'>
                                                <button className="ml-[80%] font-bold text-blue-500 py-1 px-4 rounded"
                                                    onClick={(e) => toggleComment(e, data)}>
                                                    Read More...
                                                </button>
                                            </div>
                        ...
                        <div className="relative w-full max-w-[100vw] mx-auto bg-white rounded-md shadow-lg h-full">
                            ...
                            <div className='bg-gray-100 m-auto w-[80%] justify-center'>
                                <div className='flex flex-col w-[80%] m-auto'>
                                    <div className='flex'>
                                        <img
                                        className='rounded-full w-[30px] h-[30px] object-cover m-2 '
                                         src={selectedPost.ID.photo ?  selectedPost.ID.photo : "https://static.vecteezy.com/system/resources/thumbnails/024/226/462/small/happy-anime-boy-logo-vector.jpg"} alt="" />
                                        <p className='ml-[10px] m-auto font-semibold'>{selectedPost.ID.name}</p>
                                    </div>
                                </div>
                                <form>
                                    <div className="w-[75%] m-auto mt-[2%] border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                                        ...
                                        <div className="flex items-center justify-between px-3 py-2 border-t dark:border-gray-600">
                                            ...
                                            <div className="flex pl-0 space-x-1 sm:pl-2">
                                                <div className="inline-flex justify-center items-center p-2 text-gray-500 rounded cursor-pointer hover:text-gray-900 hover:bg-gray-100 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600">
                                                    <span>{new Date(selectedPost.updatedAt).toLocaleString()}</span>
                                                </div>
                                            </div>
                                        </div>
