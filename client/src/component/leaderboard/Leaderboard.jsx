...
export default function Leaderboard({data}) {
  ...
  return (	
   <>
   <div className="px-12 h-80 overflow-y-scroll mt-2 space-y-2 w-80  ">
  {sortedData.map((item, index) => (
    <div className="flex items-center py-1 space-x-4" key={index}>
      <img
        src={item.ID.photo}
        alt=""
      ...
      <div className="w-[100%]">
        <div className="flex justify-between w-[100%]">
          <h2 className="text-lg font-semibold">{item.ID.name}</h2>
          <span>{(item?.points?.[0]?.point_curr + item?.points?.[0]?.profile_v + item?.points?.[0]?.externalPoint)|| 0}</span>
        </div>
